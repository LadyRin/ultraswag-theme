...
function activate(context) {
	this.extensionName = 'LadyRin.ultraswag-theme';
	this.cntx = context;

	const config = vscode.workspace.getConfiguration('ultraswag');
	...
	let disposable = vscode.commands.registerCommand('ultraswag-theme.enableGlow', function () {
		...
		try {
			...
			if (!isEnabled) {
				...
				let output = html.replace(/^.*(<!-- ULTRASWAG --><script src="neondreams.js"><\/script><!-- NEON DREAMS -->).*\n?/mg, '');
				// add script tag
				output = html.replace(/\<\/html\>/g, `	<!-- ULTRASWAG --><script src="neondreams.js"></script><!-- NEON DREAMS -->\n`);
				output += '</html>';
	...
	let disable = vscode.commands.registerCommand('ultraswag-theme.disableGlow', uninstall);
...
function uninstall() {
	...
	if (isEnabled) {
		...
		vscode.window
			.showInformationMessage("htmlFile: " + htmlFile + "Neon Dreams disabled. VS code must reload for this change to take effect htmlFile", { title: "Restart editor to complete" })
			.then(function(msg) {
				vscode.commands.executeCommand("workbench.action.reloadWindow");
			});
	} else {
